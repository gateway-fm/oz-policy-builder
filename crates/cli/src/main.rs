...
/// directly against regeneration. Emission produces a flat `src/`, so this does not descend —
/// recursing would invent keys regeneration never emits, and every one of them would then read
/// as a mismatch.
///
/// A subdirectory is **refused**, not skipped. Skipping it let `verify` report a reproduction
/// match over a tree containing Rust files it had not looked at. Nothing undeclared can reach
/// the wasm — a module has to be named in `lib.rs` or `contract.rs`, and both of those are
/// compared — but "I did not read that" and "that matches" are different answers and only one of
/// them was true.
pub(crate) fn read_sources(
    ...
    for entry in std::fs::read_dir(dir).with_context(|| format!("reading {}", dir.display()))? {
        ...
        if path.is_dir() {
            anyhow::bail!(
                "{} contains a subdirectory, {}; a generated crate's src/ is flat, so this is \
                 not a generated crate and verify would otherwise report a match over files it \
                 never read",
                dir.display(),
                path.display()
            );
...
mod tests {
    ...
    /// `read_sources` reads a flat `src/` and refuses anything else.
    ///
    /// The refusal is the half worth testing. `verify` compares this map against regeneration, so
    /// a subdirectory that was silently skipped turned "there are Rust files here I did not read"
    /// into "this crate reproduces" — and the two are not the same answer. Nothing undeclared can
    /// reach the wasm, since a module has to be named in a file that *is* compared, which is why
    /// this is a reporting defect rather than a soundness one; a reporting defect in the one
    /// command whose entire output is a yes or a no is still worth closing.
    #[test]
    fn read_sources_refuses_a_source_tree_it_cannot_read_flatly() {
        let dir = std::env::temp_dir().join(format!("ozpb-read-sources-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("a scratch directory");

        std::fs::write(dir.join("lib.rs"), "#![no_std]\n").expect("writing lib.rs");
        std::fs::write(dir.join("contract.rs"), "// the rule\n").expect("writing contract.rs");
        std::fs::write(dir.join("notes.txt"), "not rust\n").expect("writing a non-source file");

        let flat = read_sources(&dir).expect("a flat src/ must be readable");
        assert_eq!(
            flat.keys().collect::<Vec<_>>(),
            vec!["src/contract.rs", "src/lib.rs"],
            "keys must be the paths the generated file set uses, and non-Rust files ignored"
        );

        std::fs::create_dir_all(dir.join("extra")).expect("a subdirectory");
        std::fs::write(dir.join("extra/mod.rs"), "// more code\n").expect("writing extra/mod.rs");
        let error = read_sources(&dir).expect_err("a subdirectory must be refused, not skipped");
        assert!(
